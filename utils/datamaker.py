...
def get_data(config, root_dir="/dataset/"):
    ...
    instance_count = 0
    x_batch = np.zeros(
        [config["BATCH_SIZE"], config["IMAGE_W"], config["IMAGE_H"], 3], np.float32)
    ...
    for file in os.listdir(os.getcwd() + root_dir + "images/"):
        ...
        labels_all = [l + labels_file[i - 1]
                      for i, l in enumerate(labels_file) if i % 2 == 0]
        ...
        for obj in objs:
            ...
            center_w = (obj.xmax - obj.xmin) / \
                (config["IMAGE_W"] / config["GRID_W"])
            center_h = (obj.ymax - obj.ymin) / \
                (config["IMAGE_H"] / config["GRID_H"])

            # print "centerx = {} centery = {} centerw = {} centerh =
            # {}".format(center_x, center_y, center_w, center_h)
            ...
            y_batch[instance_count, grid_x, grid_y, best_prior,
                    5:5 + config["CLASS"]] = class_vector
