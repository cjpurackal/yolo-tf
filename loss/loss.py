...
def yolo_loss(y_pred, config, y_true):
    ...
    cell_x = tf.to_float(
        tf.reshape(
            tf.tile(
                tf.range(
                    config["GRID_W"]), [
                    config["GRID_H"]]), (1, config["GRID_H"], config["GRID_W"], 1, 1)))
    cell_y = tf.transpose(cell_x, (0, 2, 1, 3, 4))
    cell_grid = tf.tile(tf.concat(
        [cell_x, cell_y], -1), [config["BATCH_SIZE"], 1, 1, config["BOX"], 1])
    ...
    intersection_wh = tf.maximum(
        intersection_cords_maxs -
        intersection_cords_mins,
        0)
    ...
    conf_mask = conf_mask + \
        tf.to_float(iou_scores < 0.6) * (1 - y_true[..., 4]) * config["NO_OBJECT_SCALE"]

    conf_mask = conf_mask + y_true[..., 4] * config["OBJECT_SCALE"]
    class_mask = y_true[..., 4] * \
        tf.gather(class_wt, true_classes) * config["CLASS_SCALE"]
    coord_mask = tf.expand_dims(
        y_true[..., 4], axis=-1) * config["COORD_SCALE"]
    ...
    loss_xy = tf.reduce_sum(tf.square(pred_xy - true_xy)
                            * coord_mask) / (nb_coord_box + 1e-6) / 2.
    loss_wh = tf.reduce_sum(tf.square(pred_wh - true_wh)
                            * coord_mask) / (nb_coord_box + 1e-6) / 2.
    loss_conf = tf.reduce_sum(tf.square(
        true_box_conf - pred_box_conf) * conf_mask) / (nb_conf_box + 1e-6) / 2.
    loss_class = tf.nn.sparse_softmax_cross_entropy_with_logits(
        labels=true_classes, logits=pred_classes)
