...
class Arch:
    ...
    def darknet(self):
        conv1 = tf.layers.conv2d(
            self.x, filters=32, kernel_size=[
                3, 3], padding="same", activation=tf.nn.leaky_relu)
        ...
        conv2 = tf.layers.conv2d(
            maxpool1, filters=64, kernel_size=[
                3, 3], padding="same", activation=tf.nn.leaky_relu)
        ...
        conv3 = tf.layers.conv2d(
            maxpool2, filters=128, kernel_size=[
                3, 3], padding="same", activation=tf.nn.leaky_relu)
        conv3 = tf.layers.batch_normalization(conv3)
        conv4 = tf.layers.conv2d(
            conv3, filters=64, kernel_size=[
                1, 1], padding="same", activation=tf.nn.leaky_relu)
        conv4 = tf.layers.batch_normalization(conv4)
        conv5 = tf.layers.conv2d(
            conv4, filters=128, kernel_size=[
                3, 3], padding="same", activation=tf.nn.leaky_relu)
        ...
        conv6 = tf.layers.conv2d(
            maxpool3, filters=256, kernel_size=[
                3, 3], padding="same", activation=tf.nn.leaky_relu)
        conv6 = tf.layers.batch_normalization(conv6)
        conv7 = tf.layers.conv2d(
            conv6, filters=128, kernel_size=[
                1, 1], padding="same", activation=tf.nn.leaky_relu)
        conv7 = tf.layers.batch_normalization(conv7)
        conv8 = tf.layers.conv2d(
            conv7, filters=256, kernel_size=[
                3, 3], padding="same", activation=tf.nn.leaky_relu)
